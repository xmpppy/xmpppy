...
class NonSASL(PlugIn):
    ...
    def plugin(self, owner):
        ...
        if query.getTag('digest'):
            ...
            query.setTagData(
                'digest',
                sha1(owner.Dispatcher.Stream._document_attrs['id']+self.password).hexdigest()
            )
        ...
        if isResultNode(resp):
            self.DEBUG('Successfully authenticated with remove host.', 'ok')
...
class SASL(PlugIn):
    ...
    def FeaturesHandler(self, conn, feats):
        ...
        self._owner.RegisterHandler(
            'challenge', self.SASLHandler, xmlns=NS_SASL)
        ...
        if "ANONYMOUS" in mecs and self.username is None:
            ...
            sasl_data = '%s\x00%s\x00%s' % (
                self.username+'@'+self._owner.Server, self.username, self.password)
            payload = base64.encodestring(sasl_data.encode(
                'utf-8')).decode('utf-8').replace('\r', '').replace('\n', '')
            node = Node('auth', attrs={
                        'xmlns': NS_SASL, 'mechanism': 'PLAIN'}, payload=[payload])
        else:
            self.startsasl = 'failure'
            self.DEBUG('I can only use DIGEST-MD5 and PLAIN mechanisms.', 'error')
            return
    ...
    def SASLHandler(self, conn, challenge):
        ...
        if challenge.getName() == 'failure':
            ...
            try:
                ...
                reason = challenge
            self.DEBUG('Failed SASL authentication: %s' % reason, 'error')
            raise NodeProcessed
        ...
        if 'qop' in chal and 'auth' in [x.strip() for x in chal['qop'].split(',')]:
            ...
            for key in ['charset', 'username', 'realm', 'nonce', 'nc', 'cnonce', 'digest-uri', 'response', 'qop']:
                if key in ['nc', 'qop', 'response', 'charset']:
                    ...
                    sasl_data += '%s="%s",' % (key, resp[key])
# 3333
            node = Node('response', attrs={'xmlns': NS_SASL}, payload=[
                        base64.encodestring(sasl_data[:-1]).replace('\r', '').replace('\n', '')])
            self._owner.send(node.__str__())
        elif 'rspauth' in chal:
            self._owner.send(
                Node('response', attrs={'xmlns': NS_SASL}).__str__())
        else:
            self.startsasl = 'failure'
            self.DEBUG(
                'Failed SASL authentication: unknown challenge', 'error')
        raise NodeProcessed
...
class Bind(PlugIn):
    ...
    def Bind(self, resource=None):
        ...
        if resource:
            ...
            resource = []
        resp = self._owner.SendAndWaitForResponse(Protocol('iq', typ='set', payload=[
                                                  Node('bind', attrs={'xmlns': NS_BIND}, payload=resource)]))
        if isResultNode(resp):

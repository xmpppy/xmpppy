...
class SASL(PlugIn):
    ...
    def FeaturesHandler(self,conn,feats):
        ...
        if "ANONYMOUS" in mecs and self.username == None:
            ...
            payload = base64.encodebytes(sasl_data.encode('utf-8')).decode('utf-8').replace('\r','').replace('\n','')
...
########################################3333
        ...
        chal={}
        data=base64.decodebytes(incoming_data)
...
########################################3333
            node=Node('response',attrs={'xmlns':NS_SASL},payload=[base64.encodebytes(sasl_data[:-1]).replace('\r','').replace('\n','')])
            self._owner.send(node.__str__())

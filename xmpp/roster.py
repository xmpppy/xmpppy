...
class Roster(PlugIn):
    ...
    def RosterIqHandler(self, dis, stanza):
        ...
        self._data[self._owner.User + '@' + self._owner.Server] = {'resources': {}, 'name': None, 'ask': None,
                                                                   'subscription': None, 'groups': None, }
        self.set = 1
    ...
    def PresenceHandler(self, dis, pres):
        ...
        if not self._data.has_key(jid.getStripped()): self._data[jid.getStripped()] = {'name': None, 'ask': None,
                                                                                       'subscription': 'none',
        ...
        if not typ:
            ...
            item['resources'][jid.getResource()] = res = {'show': None, 'status': None, 'priority': '0',
                                                          'timestamp': None}
    ...
    def _getResourceData(self, jid, dataname):
        ...
        if jid.find('/') + 1:
            ...
            for r in self._data[jid]['resources'].keys():
                if int(self._data[jid]['resources'][r]['priority']) > lastpri: resource, lastpri = r, int(

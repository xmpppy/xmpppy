...
class Roster(PlugIn):
    ...
    def plugin(self, owner, request=1):
        ...
        self._owner.RegisterHandler(
            'iq', self.RosterIqHandler, 'result', NS_ROSTER)
        self._owner.RegisterHandler(
            'iq', self.RosterIqHandler, 'set', NS_ROSTER)
    ...
    def getRoster(self):
        """ Requests roster from server if necessary and returns self."""
        if not self.set:
            self.Request()
        while not self.set:
    ...
    def PresenceHandler(self, dis, pres):
        ...
        if jid.getStripped() not in self._data:
            self._data[jid.getStripped()] = {'name': None, 'ask': None, 'subscription': 'none', 'groups': [
                'Not in roster'], 'resources': {}}

        item = self._data[jid.getStripped()]
        typ = pres.getType()

        if not typ:
            self.DEBUG('Setting roster item %s for resource %s...' %
                       (jid.getStripped(), jid.getResource()), 'ok')
            item['resources'][jid.getResource()] = res = {
                'show': None, 'status': None, 'priority': '0', 'timestamp': None}
            if pres.getTag('show'):
        ...
        if jid.find('/') + 1:
            ...
            for r in list(self._data[jid]['resources'].keys()):
                if int(self._data[jid]['resources'][r]['priority']) > lastpri:
                    resource, lastpri = r, int(
                        self._data[jid]['resources'][r]['priority'])
            return self._data[jid]['resources'][resource][dataname]

    def delItem(self, jid):
        """ Delete contact 'jid' from roster."""
        self._owner.send(Iq('set', NS_ROSTER, payload=[Node(

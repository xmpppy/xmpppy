...
class Node(object):
    ...
    def __init__(self, tag=None, attrs={}, payload=[], parent=None, nsp=None, node_built=False, node=None):
        ...
        if node:
            ...
            if not isinstance(node, Node):
                ...
                self.name, self.namespace, self.attrs, self.data, self.kids, self.parent, self.nsd = node.name, node.namespace, {
                }, [], [], node.parent, {}
                ...
                for k, v in list(node.nsd.items()):
                    self.nsd[k] = v
        else:
    ...
    def addChild(self, name=None, attrs={}, payload=[], namespace=None, node=None):
        ...
        if 'xmlns' in attrs:
            raise AttributeError(
                "Use namespace=x instead of attrs={'xmlns':x}")
        if node:
            newnode = node
            node.parent = self
        else:
    ...
    def getTagData(self, tag):
        """ Returns concatenated CDATA of the child with specified name."""
        try:
...
class NodeBuilder:
    ...
    def starttag(self, tag, attrs):
        ...
        self.check_data_buffer()
        self._inc_depth()
        self.DEBUG(DBG_NODEBUILDER, "DEPTH -> %i , tag -> %s, attrs -> %s" %
                   (self.__depth, tag, repr(attrs)), 'down')

            ...
            for query in possible_queries:
                try:
                    if HAVE_DNSPYTHON:
                        ...
                        # Sort by priority, according to RFC 2782.
                        answers = sorted(response.answers,
                                         key=lambda a: a['data'][0])
                        if len(answers) > 0:
                    ...
                    self.DEBUG('An error occurred while looking up %s' %
                               query, 'warn')
            server = (host, port)
        else:
        ...
        try:
            for res in socket.getaddrinfo(server[0], int(server[1]), 0, socket.SOCK_STREAM):
                ...
                try:
                    ...
                    self.DEBUG("Successfully connected to remote host %s" %
                               repr(server), 'start')
                    return 'ok'
                    ...
                    if self._sock is not None:
                        self._sock.close()
            self.DEBUG("Failed to connect to remote host %s: %s (%s)" %
                       (repr(server), strerror, errno), 'error')
        except socket.gaierror as xxx_todo_changeme1:
            (errno, strerror) = xxx_todo_changeme1.args
            self.DEBUG("Failed to lookup remote host %s: %s (%s)" %
                       (repr(server), strerror, errno), 'error')
    ...
    def check_pending(self, ex, direction, action):
        if hasattr(socket, 'sslerror'):
            if ex[0] == socket.SSL_ERROR_WANT_READ:
                sys.exc_clear()
                self.DEBUG("SSL_WANT_READ while {direction} data, {action}".format(
                    **locals()), 'warn')
                return True
            if ex[0] == socket.SSL_ERROR_WANT_WRITE:
                sys.exc_clear()
                self.DEBUG("SSL_WANT_WRITE while {direction} data, {action}".format(
                    **locals()), 'warn')
                return True
        else:
...
class HTTPPROXYsocket(TCPsocket):
    ...
    def connect(self, server=None):
        ...
        if not TCPsocket.connect(self, (self._proxy['host'], self._proxy['port'])):
            return
        self.DEBUG("Proxy server contacted, performing authentication", 'start')
        if not server:
            server = self._server
        ...
        while reply.find('\n\n') == -1:
            try:
                ...
                self._owner.disconnected()
                return
        self.DEBUG("Authentication successfull. XMPP server contacted.", 'ok')
        return 'ok'
...
class TLS(PlugIn):
    """ TLS connection used to encrypts already established tcp connection."""

    def PlugIn(self, owner, now=0):
        """ If the 'now' argument is true then starts using encryption immediately.
    ...
    def plugout(self, now=0):
        ...
        self._owner.UnregisterHandler(
            'features', self.FeaturesHandler, xmlns=NS_STREAMS)
        self._owner.UnregisterHandler(
            'proceed', self.StartTLSHandler, xmlns=NS_TLS)
        self._owner.UnregisterHandler(
            'failure', self.StartTLSHandler, xmlns=NS_TLS)
...
class Bosh(PlugIn):
    ...
    def plugin(self, owner):
        # XXX Provide reasonable defaults if non were given, lookup service
    ...
    def receive(self):
        ...
        if res.status == OK:
            ...
            self.DEBUG("The server did not understand the request")
            ...
            msg = "Received status not defined in XEP-1204: %s" % res.status
            self.DEBUG(msg, 'error')
            raise Exception("Disconnected from server")
        node = Node(node=data)
    ...
    def pending_data(self, timeout=0):
        ...
        if self.PIPELINE:
            ...
            pending = select.select(
                list(self._respobjs.keys()), [], [], timeout,)[0]

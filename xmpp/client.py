    ...
    def PlugIn(self, owner):
        ...
        for method in self._exported_methods:
            if method.__name__ in owner.__dict__:
                self._old_owners_methods.append(
                    owner.__dict__[method.__name__])
...
class CommonClient:
    ...
    def __init__(self, server, port=5222, debug=['always', 'nodebuilder']):
        """ Caches server name and (optionally) port to connect to. "debug" parameter specifies
            the debug IDs that will go into debug output. You can either specify an "include"
    ...
    def DisconnectHandler(self):
        """ Default disconnect handler. Just raises an IOError.
            If you choose to use this class in your production client,
...
class Client(CommonClient):
    ...
    def connect(self, server=None, proxy=None, secure=None, use_srv=True, transport=None):
        ...
        if not CommonClient.connect(self, server, proxy, secure, use_srv, transport) or secure is not None and not secure:
            return self.connected
    ...
    def auth(self, user, password, resource='', sasl=1):
        """ Authenticate connection and bind resource. If resource is not provided
        ...
        if 'version' in self.Dispatcher.Stream._document_attrs and self.Dispatcher.Stream._document_attrs['version'] == '1.0':
            while not self.Dispatcher.Stream.features and self.Process(1):
                pass      # If we get version 1.0 stream the features tag MUST BE presented
        if sasl:
            xmpp.auth.SASL(user, password).PlugIn(self)
...
class Component(CommonClient):
    ...
    def auth(self, name, password, dup=None):
        ...
        try:
            if self.sasl:
                xmpp.auth.SASL(name, password).PlugIn(self)

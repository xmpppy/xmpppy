...
class CommonClient(object):
    ...
    def __init__(self, server, port=5222, debug=('always', 'nodebuilder')):
        ...
        Full list: ['nodebuilder', 'dispatcher', 'gen_auth', 'SASL_auth',
        'bind', 'socket', 'CONNECTproxy', 'TLS', 'roster', 'browser', 'ibb'].
        """
    ...
    def disconnected(self):
        ...
        [handler() for handler in self.disconnect_handlers[::-1]]
    ...
    def reconnectAndReauth(self):
        ...
        dumped_handlers = self.Dispatcher.dumpHandlers()
        ...
        self.Dispatcher.restoreHandlers(dumped_handlers)
        return self.connected

    def connect(self, server=None, proxy=None, ssl=None, use_srv=None):
        """ Make a tcp/ip connection, protect it with tls/ssl
        if possible and start XMPP stream.
        Returns None or 'tcp' or 'tls', depending on the result.
        """
        server = server or (self.Server, self.Port)
        if proxy:
...
class Client(CommonClient):
    ...
    def connect(self, server=None, proxy=None, secure=None, use_srv=True):
        ...
        If you want ssl/tls support to be discovered and enable automatically - leave third argument as None.
        (ssl will be auto-detected only if port is 5223 or 443)
        ...
        """
        connected = CommonClient.connect(self, server, proxy, secure, use_srv)
        use_secure = secure is None or secure
        if not connected or not use_secure:
            return self.connected
        ...
        if not self.Dispatcher.Stream.features.getTag(
                'starttls'): return self.connected       # TLS not supported by server

...
class Dispatcher(PlugIn):
    ...
    def plugin(self, owner):
        """ Plug the Dispatcher instance into
            Client class instance and send initial stream header.
            Used internally.
        """
        self._init()
    ...
    def StreamInit(self):
        ...
        self._owner.send("<?xml version='1.0'?>%s>" %
    ...
    def Process(self, timeout=0):
        ...
        if len(self._pendingExceptions) > 0:
            ...
            raise _pendingException[0](
                _pendingException[1]).with_traceback(_pendingException[2])
    ...
    def RegisterNamespaceHandler(self, xmlns, handler, typ='', ns='', makefirst=0, system=0):
        ...
        self.RegisterHandler('default', handler, typ, ns,
                             xmlns, makefirst, system)
    ...
    def Event(self, realm, event, data):
        """ Raise some event. Takes three arguments:
            ...
            2) "event" - the event itself. e.g. "SUCCESSFULL SEND".
    ...
    def dispatch(self, stanza, session=None, direct=0):
        ...
        session.DEBUG("Dispatching %s stanza with type->%s props->%s id->%s" %
                      (name, typ, stanza.props, ID), 'ok')
        ...
        if ID in session._expected:
            user = 0
            if isinstance(session._expected[ID], ()):
    ...
    def WaitForResponse(self, ID, timeout=DefaultTimeout):
        ...
        self.DEBUG("Waiting for ID:%s with timeout %s..." %
                   (ID, timeout), 'wait')

...
class Browser(PlugIn):
    """ WARNING! This class is for components only. It will not work in client mode!
        ...
        info should be a dictionary and must have keys 'ids' and 'features'.
    ...
    def __init__(self):
        """Initializes internal variables. Used internally."""
    ...
    def plugin(self, owner):
        ...
        owner.RegisterHandler(
            'iq', self._DiscoveryHandler, typ='get', ns=NS_DISCO_INFO)
        owner.RegisterHandler(
            'iq', self._DiscoveryHandler, typ='get', ns=NS_DISCO_ITEMS)
    ...
    def getDiscoHandler(self, node='', jid=''):
        """ Returns the previously registered DISCO handler
            that is responsible for this node/jid combination.
            Used internally."""
    ...
    def delDiscoHandler(self, node='', jid=''):
        """ Unregisters DISCO handler that is responsible for this
    ...
    def _DiscoveryHandler(self, conn, request):
        ...
        if not handler:
            self.DEBUG("No Handler for request with jid->%s node->%s ns->%s" % (request.getTo().__str__(
            ).encode('utf8'), nodestr.encode('utf8'), request.getQueryNS().encode('utf8')), 'error')
            conn.send(Error(request, ERR_ITEM_NOT_FOUND))
            raise NodeProcessed
        self.DEBUG("Handling request with jid->%s node->%s ns->%s" % (request.getTo().__str__(
        ).encode('utf8'), nodestr.encode('utf8'), request.getQueryNS().encode('utf8')), 'ok')

...
class IBB(PlugIn):
    ...
    def __init__(self):
        ...
        self._ampnode = Node(NS_AMP + ' amp',
                             payload=[Node('rule', {'condition': 'deliver-at', 'value': 'stored', 'action': 'error'}),
                                      Node('rule',
                                           {'condition': 'match-resource', 'value': 'exact', 'action': 'error'})])
...
"""
        ...
        if err:
            ...
            self._streams[sid] = {'direction': '<' + str(stanza.getFrom()), 'block-size': blocksize,
                                  'fp': open('/tmp/xmpp_file_' + sid, 'w'), 'seq': 0, 'syn_id': stanza.getID()}
        conn.send(rep)
        ...
        for sid in self._streams.keys():
            ...
            if stream['direction'][:2] == '|>':
                ...
                if chunk:
                    ...
                    conn.send(
                        Protocol('iq', stream['direction'][1:], 'set', payload=[Node(NS_IBB + ' close', {'sid': sid})]))
...
"""
    ...
    def ReceiveHandler(self, conn, stanza):
        ...
        if err:
            ...
            conn.send(
                Error(Iq(to=stanza.getFrom(), frm=stanza.getTo(), payload=[Node(NS_IBB + ' close')]), err, reply=0))

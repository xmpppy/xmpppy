...
NS_ADMIN_GET_USER_PASSWORD = NS_ADMIN + \
    '#get-user-password'              # XEP-0133
NS_ADMIN_CHANGE_USER_PASSWORD = NS_ADMIN + \
    '#change-user-password'           # XEP-0133
NS_ADMIN_GET_USER_ROSTER = NS_ADMIN + '#get-user-roster'                # XEP-0133
NS_ADMIN_GET_USER_LASTLOGIN = NS_ADMIN + \
    '#get-user-lastlogin'             # XEP-0133
...
NS_ADMIN_REGISTERED_USERS_NUM = NS_ADMIN + \
    '#get-registered-users-num'       # XEP-0133
NS_ADMIN_DISABLED_USERS_NUM = NS_ADMIN + \
    '#get-disabled-users-num'         # XEP-0133
...
NS_ADMIN_REGISTERED_USERS_LIST = NS_ADMIN + \
    '#get-registered-users-list'      # XEP-0133
NS_ADMIN_DISABLED_USERS_LIST = NS_ADMIN + \
    '#get-disabled-users-list'        # XEP-0133
NS_ADMIN_ONLINE_USERS_LIST = NS_ADMIN + \
    '#get-online-users-list'          # XEP-0133
NS_ADMIN_ACTIVE_USERS_LIST = NS_ADMIN + \
    '#get-active-users-list'          # XEP-0133
...
class Protocol(Node):
    ...
    def __init__(self, name=None, to=None, typ=None, frm=None, attrs={}, payload=[], timestamp=None, xmlns=None, node=None):
        ...
        if node and type(self) == type(node) and self.__class__ == node.__class__ and 'id' in self.attrs:
            del self.attrs['id']
        self.timestamp = None
        ...
        if timestamp is not None:
            # To auto-timestamp stanza just pass timestamp=''
    ...
    def setError(self, error, code=None):
        ...
        if code:
            if str(code) in list(_errorcodes.keys()):
                ...
                error = ErrorNode(ERR_UNDEFINED_CONDITION,
                                  code=code, typ='cancel', text=error)
...
class Message(Protocol):
    ...
    def buildReply(self, text=None):
        ...
        m = Message(to=self.getFrom(), frm=self.getTo(),
                    body=text, typ=self.getType())
        th = self.getThread()
        if th:
            m.setThread(th)
        return m
...
class Iq(Protocol):
    ...
    def __init__(self, typ=None, queryNS=None, attrs={}, to=None, frm=None, payload=[], xmlns=NS_CLIENT, node=None):
        ...
        Protocol.__init__(self, 'iq', to=to, typ=typ,
                          attrs=attrs, frm=frm, xmlns=xmlns, node=node)
        if payload:
...
class Error(Protocol):
    ...
    def __init__(self, node, error, reply=1):
        ...
        if reply:
            Protocol.__init__(self, to=node.getFrom(),
                              frm=node.getTo(), node=node)
        else:
...
class DataField(Node):
    ...
    def addOption(self, opt):
        ...
        if type(opt) in [str, str]:
            ...
            self.addChild('option', {'label': opt[0]}).setTagData(
                'value', opt[1])

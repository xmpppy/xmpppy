...
class Session:
    ...
    Instead you should use poll() (recommended) or select() methods for this purpose.
    ...
    Session instance have multitude of internal attributes. The most important is the 'peer' one.
    ...
    def sendnow(self, chunk):
        """ Put chunk into "immediately send" queue.
            Should only be used for auth/TLS stuff and like.
            If you just want to schedule regular stanza for delivery use enqueue method.
        """
    ...
    def push_queue(self, failreason=ERR_RECIPIENT_UNAVAILABLE):
        ...
        if self._stream_state >= STREAM__CLOSED or self._socket_state >= SOCKET_DEAD:
            self._owner.deactivatesession(self)
            for key in self.deliver_key_queue:                                          # Not sure. May be I
                # should simply re-dispatch it?
                self._dispatch(
                    Error(self.deliver_queue_map[key], failreason), trusted=1)
            for stanza in self.stanza_queue:                                            # But such action can invoke
                # Infinite loops in case of S2S connection...
                self._dispatch(Error(stanza, failreason), trusted=1)
            ...
            for stanza in self.stanza_queue:
                ...
                self.deliver_queue_map[self._stream_pos_queued] = stanza
                self.deliver_key_queue.append(self._stream_pos_queued)
            self.stanza_queue = []
            # UNLOCK_QUEUE

    def flush_queue(self):
        """ Put the "immediately send" queue content on the wire. Blocks until at least one byte sent."""
        if self.sendbuffer:
            try:
                # LOCK_QUEUE
            ...
            self._stream_pos_delivered = self._stream_pos_sent
    ...
    def _dispatch(self, stanza, trusted=0):
        ...
        if self._stream_state == STREAM__OPENED or trusted:
    ...
    def start_feature(self, f):
        ...
        if self.feature_in_process:
            raise Exception("Starting feature %s over %s !" %
                            (f, self.feature_in_process))
    ...
    def stop_feature(self, f):
        ...
        if self.feature_in_process != f:
            raise Exception("Stopping feature %s instead of %s !" %
                            (f, self.feature_in_process))
        self.feature_in_process = None
    ...
    def set_session_state(self, newstate):
        ...
        if self._session_state < newstate:
            if self._session_state < SESSION_AUTHED and newstate >= SESSION_AUTHED:

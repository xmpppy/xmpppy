...
def send_message(jabberid, password, receiver, message, debug=False):
    ...
    connection.connect()
    connection.auth(user=jid.getNode(), password=password,
...
def simple_message():
    parser = argparse.ArgumentParser(
        description='Authenticate with XMPP server and send simple message')
    parser.add_argument('--jabberid', required=True,
                        help='Jabber Identifier (JID) aka. username')
    parser.add_argument('--password', required=True,
                        help='Password for Jabber Identifier (JID)')
    ...
    send_message(options.jabberid, options.password,
                 options.receiver, options.message, debug=options.debug)

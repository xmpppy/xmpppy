...
class Command_Handler_Prototype(PlugIn):
    ...
    def __init__(self, jid=''):
        ...
        self.discoinfo = {'ids': [{'category': 'automation', 'type': 'command-node', 'name': self.description}],
...
class TestCommand(Command_Handler_Prototype):
    ...
    def cmdFirstStage(self, conn, request):
        ...
        if session == None:
            ...
            self.sessions[session] = {'jid': request.getFrom(),
                                      'actions': {'cancel': self.cmdCancel, 'next': self.cmdSecondStage,
                                                  'execute': self.cmdSecondStage}, 'data': {'type': None}}
        ...
        form = DataForm(title='Select type of operation', data=[
            ...
            DataField(name='calctype', desc='Calculation Type', value=self.sessions[session]['data']['type'],
                      options=[['circlediameter', 'Calculate the Diameter of a circle'],
                               ['circlearea', 'Calculate the area of a circle']], typ='list-single', required=1)])
        replypayload = [Node('actions', attrs={'execute': 'next'}, payload=[Node('next')]), form]
        reply.addChild(name='command', namespace=NS_COMMANDS,
                       attrs={'node': request.getTagAttr('command', 'node'), 'sessionid': session,
                              'status': 'executing'}, payload=replypayload)
    ...
    def cmdSecondStage(self, conn, request):
        ...
        self.sessions[request.getTagAttr('command', 'sessionid')]['actions'] = {'cancel': self.cmdCancel,
    ...
    def cmdSecondStageReply(self, conn, request):
        reply = request.buildReply('result')
        form = DataForm(title='Enter the radius', data=['Enter the radius of the circle (numbers only)',
        ...
        reply.addChild(name='command', namespace=NS_COMMANDS, attrs={'node': request.getTagAttr('command', 'node'),
                                                                     'sessionid': request.getTagAttr('command',
                                                                                                     'sessionid'),
                                                                     'status': 'executing'}, payload=replypayload)
        self._owner.send(reply)
        raise NodeProcessed

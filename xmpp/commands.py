...
To use this module:
    ...
    The command feature must be added to existing disco replies where necessary.
...
class Commands(PlugIn):
    ...
    The commands class provides a lookup and browse mechanism. It follows the same priciple of the Browser class, for Service Discovery to provide the list of commands, it adds the 'list' disco type to your existing disco handler function.
    ...
    def __init__(self, browser):
        """Initializes class and sets up local variables"""
    ...
    def plugin(self, owner):
        ...
        owner.RegisterHandler(
            'iq', self._CommandHandler, typ='set', ns=NS_COMMANDS)
        owner.RegisterHandler(
            'iq', self._CommandHandler, typ='get', ns=NS_COMMANDS)
        self._browser.setDiscoHandler(
            self._DiscoHandler, node=NS_COMMANDS, jid='')

    def plugout(self):
        """Removes handlers from the session"""
        # unPlug from the session and the disco manager
        self._owner.UnregisterHandler(
            'iq', self._CommandHandler, ns=NS_COMMANDS)
    ...
    def _DiscoHandler(self, conn, request, typ):
        ...
        if typ == 'items':
            ...
            if items != []:
                for each in items:
                    i = self._handlers[each[0]][each[1]]['disco'](
                        conn, request, 'list')
                    if i is not None:
                        list.append(
                            Node(tag='item', attrs={'jid': i[0], 'node': i[1], 'name': i[2]}))
    ...
    def addCommand(self, name, cmddisco, cmdexecute, jid=''):
        ...
        if jid not in self._handlers:
            self._handlers[jid] = {}
            self._browser.setDiscoHandler(
                self._DiscoHandler, node=NS_COMMANDS, jid=jid)
        if name in self._handlers[jid]:
            raise NameError('Command Exists')
        else:
            self._handlers[jid][name] = {
                'disco': cmddisco, 'execute': cmdexecute}
...
class Command_Handler_Prototype(PlugIn):
    ...
    def __init__(self, jid=''):
        ...
        self.discoinfo = {'ids': [{'category': 'automation', 'type': 'command-node',
                                   'name': self.description}], 'features': self.discofeatures}
        self._jid = jid
    ...
    def plugin(self, owner):
        ...
        self._commands.addCommand(
            self.name, self._DiscoHandler, self.Execute, jid=self._jid)
...
class TestCommand(Command_Handler_Prototype):
    ...
    def cmdFirstStage(self, conn, request):
        ...
        form = DataForm(title='Select type of operation', data=['Use the combobox to select the type of calculation you would like to do, then click Next', DataField(
            name='calctype', desc='Calculation Type', value=self.sessions[session]['data']['type'], options=[['circlediameter', 'Calculate the Diameter of a circle'], ['circlearea', 'Calculate the area of a circle']], typ='list-single', required=1)])
        replypayload = [
            Node('actions', attrs={'execute': 'next'}, payload=[Node('next')]), form]
        reply.addChild(name='command', namespace=NS_COMMANDS, attrs={'node': request.getTagAttr(
            'command', 'node'), 'sessionid': session, 'status': 'executing'}, payload=replypayload)
        self._owner.send(reply)
        raise NodeProcessed

    def cmdSecondStage(self, conn, request):
        form = DataForm(node=request.getTag(
            name='command').getTag(name='x', namespace=NS_DATA))
        self.sessions[request.getTagAttr(
            'command', 'sessionid')]['data']['type'] = form.getField('calctype').getValue()
        self.sessions[request.getTagAttr('command', 'sessionid')]['actions'] = {
            'cancel': self.cmdCancel, None: self.cmdThirdStage, 'previous': self.cmdFirstStage, 'execute': self.cmdThirdStage, 'next': self.cmdThirdStage}
    ...
    def cmdSecondStageReply(self, conn, request):
        ...
        replypayload = [
            Node('actions', attrs={'execute': 'complete'}, payload=[Node('complete'), Node('prev')]), form]
        reply.addChild(
            name='command',
    ...
    def cmdThirdStage(self, conn, request):
        ...
        if self.sessions[request.getTagAttr('command', 'sessionid')]['data']['type'] == 'circlearea':
            ...
            result = num*2*pi
        reply = request.buildReply('result')
        form = DataForm(typ='result', data=[DataField(
            desc='result', name='result', value=result)])
        reply.addChild(
            name='command',
            namespace=NS_COMMANDS,
            attrs={
